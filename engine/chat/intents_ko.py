...
_VERB_TABLE: Sequence[Tuple[str, str]] = (
    ...
    ("jump", r"(?:으로|로)\s*(?:가\s*줘|가줘|가\s*자|가요|가\b|가$|가\s*볼래|이동|옮겨|옮기|점프)|이동해|이동시켜|옮겨\s*줘|옮겨줘"
...
# "다 취소", "모두 되돌려": 되돌리기는 한 번에 하나라서 되묻는다
UNDO_ALL_RE = re.compile(r"(?<![가-힣])(?:다|모두|전부|싹|몽땅)\s*(?=취소|되돌)")
# "방금 파란 표시 지워": 방금 넣은 것 전체인지, 그 색 표시 모두인지 되묻는다
RECENT_RE = re.compile(r"방금|아까")
# "내가 찍은 표시": 사용자가 직접 넣은 표시 (도우미는 지우지 않는다)
OWN_MARKS_RE = re.compile(r"(?:내가|제가|내\s*손으로|직접)\s*(?:찍은|넣은|단|만든|표시한|달아\s*둔|찍어\s*둔|넣어\s*둔)")
